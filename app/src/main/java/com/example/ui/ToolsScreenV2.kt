...
fun ToolsScreenV2(viewModel: PdfAppViewModel, onBack: () -> Unit, onOpenDocument: (DocumentEntity) -> Unit) {
    ...
    val picker = rememberLauncherForActivityResult(ActivityResultContracts.OpenMultipleDocuments()) { uris ->
        if (uris.isEmpty() || selectedTool == null) return@rememberLauncherForActivityResult
        pendingUris = uris
        when (selectedTool!!.name) {
            ...
            else -> runTool(context, viewModel, selectedTool!!, uris, null, null, null, scope, { busy = it }, { busy = null; outputText = it }, onOpenDocument)
        }
    }

    val tools = listOf(
        FunctionalPdfTool("Merge PDFs", true),
        FunctionalPdfTool("Split PDF"),
        FunctionalPdfTool("Extract Pages"),
        FunctionalPdfTool("Rotate PDF"),
        FunctionalPdfTool("Protect PDF"),
        FunctionalPdfTool("Unlock PDF"),
        FunctionalPdfTool("Images to PDF", true),
        FunctionalPdfTool("PDF to Images"),
        FunctionalPdfTool("PDF to Text"),
        FunctionalPdfTool("Remove Blank"),
        FunctionalPdfTool("Watermark"),
        FunctionalPdfTool("Resize Pages"),
        FunctionalPdfTool("Print PDF")
    )
    ...
    Scaffold(topBar = {
        ...
        if (busy != null) {
            ...
            LazyVerticalGrid(GridCells.Fixed(3), Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), horizontalArrangement = Arrangement.spacedBy(12.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                items(tools) { tool ->
                    Card(onClick = {
                        selectedTool = tool
                        val types = if (tool.name == "Images to PDF") arrayOf("image/*") else arrayOf("application/pdf")
                        picker.launch(types)
                    }, modifier = Modifier.height(112.dp)) {
                        Column(Modifier.fillMaxSize().padding(8.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
                            Icon(iconFor(tool.name), null)
                            Spacer(Modifier.height(8.dp))
                            Text(tool.name, fontSize = 12.sp, textAlign = TextAlign.Center, fontWeight = FontWeight.SemiBold)
                        }
                    }
                }
            }
        ...
        if (pageDialog) AlertDialog(onDismissRequest = { pageDialog = false }, title = { Text("Extract pages") }, text = {
            OutlinedTextField(pageText, { pageText = it }, label = { Text("Page numbers (e.g. 1,3,5-7)") })
        ...
        if (passwordDialog) AlertDialog(onDismissRequest = { passwordDialog = false }, title = { Text(if (selectedTool?.name == "Protect PDF") "Protect PDF" else "Unlock PDF") }, text = {
            OutlinedTextField(passwordText, { passwordText = it }, label = { Text(if (selectedTool?.name == "Protect PDF") "Password" else "Password") })
        }, confirmButton = { Button(onClick = {
        ...
        }, enabled = passwordText.isNotBlank()) { Text("Continue") } }, dismissButton = { TextButton(onClick = { passwordDialog = false }) { Text("Cancel") } })

        if (resizeDialog) AlertDialog(onDismissRequest = { resizeDialog = false }, title = { Text("Resize pages") }, text = {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Text("Choose the output page size")
...
private fun iconFor(name: String) = when (name) {
    "Merge PDFs" -> Icons.Default.MergeType
    "Split PDF" -> Icons.Default.CallSplit
    "Extract Pages" -> Icons.Default.FilterNone
    "Rotate PDF" -> Icons.Default.RotateRight
    "Protect PDF" -> Icons.Default.Lock
    "Unlock PDF" -> Icons.Default.LockOpen
    "Images to PDF" -> Icons.Default.Image
    "PDF to Images" -> Icons.Default.Image
    "PDF to Text" -> Icons.Default.TextFields
    "Remove Blank" -> Icons.Default.DeleteOutline
    "Watermark" -> Icons.Default.WaterDrop
    "Resize Pages" -> Icons.Default.Transform
...
private fun parsePageSpec(spec: String): List<Int> {
    ...
    spec.split(',').map { it.trim() }.filter { it.isNotEmpty() }.forEach { token ->
        if (token.contains('-')) {
            ...
            val start = p[0].toIntOrNull()
            val end = p[1].toIntOrNull()
            if (start != null && end != null && start > 0 && end >= start) result += (start..end).toList()
        } else token.toIntOrNull()?.takeIf { it > 0 }?.let { result += it }
    }
        ...
        try {
            when (tool.name) {
                "Merge PDFs" -> {
                    ...
                    registerAndOpen(viewModel, file, "Merged PDF", onOpenDocument)
                    setResult("Merged ${uris.size} PDFs successfully.")
                }
                "Split PDF" -> {
                    val files = PdfToolsEngine.splitPdf(context, uris.first(), outDir)
                    if (files.isEmpty()) error("Split failed")
                    files.forEach { viewModel.repository.registerGeneratedPdf(it, "Split") }
                    setResult("Split complete: ${files.size} PDF files created in your PDF Studio library.")
                }
                "Extract Pages" -> {
                    val nums = parsePageSpec(pages ?: "")
                    if (nums.isEmpty()) error("Enter valid page numbers")
                    val file = PdfToolsAdvancedEngine.extractPages(context, uris.first(), nums, outDir) ?: error("No valid pages found")
                    registerAndOpen(viewModel, file, "Extracted", onOpenDocument)
                    setResult("Extracted ${nums.size} page(s) successfully.")
                }
                "Rotate PDF" -> {
                    val file = PdfToolsEngine.rotatePdf(context, uris.first(), 90, outDir) ?: error("Rotation failed")
                    registerAndOpen(viewModel, file, "Rotated", onOpenDocument)
                    setResult("Rotated all pages 90° clockwise.")
                }
                "Protect PDF" -> {
                    val file = PdfToolsAdvancedEngine.protectPdf(context, uris.first(), password!!, password, outDir) ?: error("Protection failed")
                    registerAndOpen(viewModel, file, "Protected, Encrypted", onOpenDocument)
                    setResult("PDF encrypted successfully. The password is required to open it.")
                }
                "Unlock PDF" -> {
                    ...
                    registerAndOpen(viewModel, file, "Unlocked", onOpenDocument)
                    setResult("PDF decrypted successfully.")
                }
                "Images to PDF" -> {
                    val file = PdfToolsAdvancedEngine.imagesToPdf(context, uris, outDir) ?: error("Could not create PDF")
                    registerAndOpen(viewModel, file, "Images to PDF", onOpenDocument)
                    setResult("Created a PDF from ${uris.size} image(s).")
                }
                "PDF to Images" -> {
                    val files = PdfToolsAdvancedEngine.renderPdfToImages(context, uris.first(), outDir)
                    if (files.isEmpty()) error("Could not render PDF")
                ...
                "Remove Blank" -> {
                    ...
                    registerAndOpen(viewModel, file, "No Blank Pages", onOpenDocument)
                    setResult("Blank pages removed successfully.")
                }
                "Watermark" -> {
                    val file = PdfToolsEngine.addWatermark(context, uris.first(), "CONFIDENTIAL", outDir) ?: error("Watermark failed")
                    registerAndOpen(viewModel, file, "Watermarked", onOpenDocument)
                    setResult("CONFIDENTIAL watermark added to every page.")
                }
                "Resize Pages" -> {
                    val file = PdfToolsAdvancedEngine.resizePages(context, uris.first(), resize ?: "A4", outDir) ?: error("Resize failed")
                    registerAndOpen(viewModel, file, "Resized", onOpenDocument)
                    setResult("Pages resized to ${resize ?: "A4"}.")
                }
                "Print PDF" -> {
                    PdfToolsEngine.printPdf(context, uris.first(), "PDF Studio Print")
                    setResult("Print dialog opened.")
                }
            }
        } catch (e: Exception) {
            setResult("${tool.name} failed: ${e.message ?: "Unknown error"}")
        } finally { setBusy("") }
    }
}
