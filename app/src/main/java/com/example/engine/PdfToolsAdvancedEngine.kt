                ...
                PDDocument.load(input).use { source ->
                    ...
                    pageNumbers.distinct().sorted().forEach { oneBased ->
                        ...
                        if (index in 0 until source.numberOfPages) {
                            // Import the page by cloning its COS dictionary so the output owns it.
                            val page = PDPage(source.getPage(index).COSObject)
                            output.addPage(page)
                        }
                    }
            ...
            context.contentResolver.openInputStream(uri)?.use { input ->
                PDDocument.load(input).use { document ->
                    for (page in document.pages) {
                        val old = page.mediaBox
                        val targetRatio = rectangle.width / rectangle.height
                        val oldRatio = old.width / old.height
                        val target = if (oldRatio > targetRatio) {
                            PDRectangle(rectangle.width, rectangle.height)
                        } else {
                            PDRectangle(rectangle.width, rectangle.height)
                        }
